...
class BookingRoomController extends Controller
{
    public function create($id)
    {
        ...
        $food = Food::with('foodCategory')->where('company_id', $companyId)->get();
        $foodCategory = FoodCategory::with('food')->where('company_id', $companyId)->get();
        ...
        if ($isBookedCount > 0) {
            ...
            try {
                ...
                foreach ($mail as $emailList) {
                    ...
                    $newParticipan->pin =  $pin;
                    $newParticipan->status_booking = 'waiting';
                    $newParticipan->save();
                    
                    $emailpin = $newParticipan->pin;
                    // $emailOrganizer = Auth::user()->email;
                    dispatch(new SendMailJob($data, $emailList, $emailpin));
