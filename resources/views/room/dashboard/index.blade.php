...
<head>
    ...
    <style>
        ...
        .pembatas1 {
            /* border-radius: 5px; */
            border-bottom: 2px solid #22df41;
        }
        
        .pembatas1:first-child {
            /* border-radius: 10px 40px 5px 5px; */
            border-bottom: 2px solid #22df41;
        }

        .pembatas1:last-child {
            border-bottom: none;
        }
        ...
        .pembatas2:first-child {
            /* border-radius: 10px 40px 5px 5px; */
            border-bottom: 2px solid #ff461f;
        }

        .pembatas2:last-child {
            border-bottom: none;
        }
...
<body class="background-display">
                ...
                <div class="col-md-5 p-0 py-3 ">
                    ...
                    <h4>
                        <div v-if="bookingOngoing.length > 0">
                            ...
                            <div class="centered mx-5 text-center text-white py-5" id="getCountdown"></div>
                            @endif
                        </div>
                    </h4>
                    <div class="centered mx-5 text-center text-white py-4" v-if="bookingOngoing.length < 1">Tidak ada meeting yang berlangsung</div>
                    <h4 class="m-0 font-weight-normal text-white">Room - {{ $rooms->room->name }}</h4>
                    <b><h1 class="m-0  text-white" v-if="bookingOngoing.length > 0">@{{ bookingOngoing[0]?.title}}</h1></b>
                    </h4>
                    <span class="badge badge-secondary m-0 font-weight-normal text-white">IP Address - @{{ ip }}</span>
                    <div id="meetingOnProgress"></div>
                    <div id="getNextMeeting"></div>
                    ...
                    <div class="logout mt-5">
                        ...
                        <a class="btn btn-success mt-auto" onclick="location.reload()">
                            <i class="fas fa-sign-out-alt mr-2"></i> {{ __('REFRESH') }}
                        </a>
                        @if ($useCame == 'YES')
                    ...
                    <div class="style-after-upcoming">
                        ...
                        <div style="height: 35vh;">
                            <div v-if="bookingTodays.length > 0">
                                <div v-if="bookingOngoing.length < 1" id="rcorners1" class="text-white h6">
                                    <p v-for="(BookToday, index) in bookingTodays" :key="index" class="pembatas1 p-2">
                                        ...
                                        &nbsp; @{{ BookToday?.for_human }} - @{{ BookToday?.start }}
                                        {{-- <span class="float-right">@{{ index + 1 }}.</span> --}}
                                    </p>
                                </div>
                                <div v-if="bookingOngoing.length > 0" id="rcorners2" class="text-white h6">
                                    <p v-for="(BookToday, index) in bookingTodays" :key="index" class="pembatas2 p-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-calendar-week" viewBox="0 0 16 16">
                                            <path d="M11 6.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1zm-3 0a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1zm-5 3a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1zm3 0a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1z" />
                                            <path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5zM1 4v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4H1z" />
                                        </svg>
            ...
            methods: {
                bookingOnGoing: function() {
                    ...
                    axios.get('{{ env('APP_URL') }}/api-display', {
                        ...
                        vm.systemOn = res.data.apiSystemOn;
                        ...
                        if (vm.device == 1 && vm.bookingOngoing[0]?.reload == 1) {
                            axios.post(`${ipVbook1}/v1/oauth2/token/`, {
                                ...
                                .then((res) => {
                                    ...
                                    axios.post(`${ipVbook1}/v1/led/front_led/`, paramsBody, config)
                                        .then((res) => {
                                            console.log(res);
                                        })
                                        .catch((err) => {
                                    ...
                                    axios.post(`${ipVbook1}/v1/led/side_led/`, paramsBody, config)
                                        .then((res) => {
                                            console.log(res);
                                        })
                                        .catch((err) => {
                            ...
                            //post reload digunakan untuk merubah reload menjadi 2
                        ...
                        // slim ditable
                        if (vm.device == 2 && vm.bookingOngoing[0]?.reload_device2 == 1) {
                            axios.post(`${ipVbook1}/v1/oauth2/token/`, {
                                    grant_type: "password",
                                    username: "admin",
                                    password: "12345678"
                            ...
                            //post reload digunakan untuk merubah reload2 menjadi 2
                            axios.post('/room/post-reload', {
                                    booking_id: this.bookingOngoing[0].id,
                                })
                                .then(function(response) {
                ...
                bookingToday: function() {
                    let vm = this;
		            axios.get('{{ env('APP_URL') }}/api-display').then(res => {
    ...
    <script>
        ...
        var systemOff = '{!! $apiSystemOff !!}';
        var alertOn = '{!! $apiAlertOn !!}';
        var alertOff = '{!! $apiAlertOff !!}';
        var x = setInterval(function() {
            ...
            // document.getElementById("getCountdown").innerHTML = hours + "h " + minutes + "m " + seconds + "s ";
            
            if (distance < 0) {
                clearInterval(x);
                document.getElementById("getCountdown").innerHTML = "MEETING SELESAI";
                var ipVbook = 'http://{{ $ip_address }}:8080';
                ...
                .then((res) => {
                    ...
                    axios.post(`${ipVbook}/v1/led/front_led/`, paramsBody, config)
                        .then((res) => {
                            console.log(res);
                        })
                        .catch((err) => {
                    ...
                    axios.post(`${ipVbook}/v1/led/side_led/`, paramsBody, config)
                        .then((res) => {
                            console.log(res);
                        })
                        .catch((err) => {
